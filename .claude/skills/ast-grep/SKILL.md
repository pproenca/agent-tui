---
name: ast-grep
description: ast-grep rule writing and usage best practices. This skill should be used when writing, reviewing, or debugging ast-grep rules for code search, linting, and transformation. Triggers on tasks involving YAML rules, pattern syntax, meta variables, constraints, or code rewriting. Also use when: (1) Creating linting rules for JavaScript/TypeScript/Python/Rust, (2) Setting up automated code transformations or refactoring, (3) Pattern-based code search and replace, (4) Debugging why an ast-grep pattern doesn't match, (5) Optimizing ast-grep performance on large codebases.
---
...
1. **Quick lookup**: Use the Quick Reference tables below to find relevant rules by category
2. **Detailed guidance**: Click any rule link to read the full reference file with examples
3. **Full context**: Read [references/full-guide.md](references/full-guide.md) for the complete 42-rule guide when you need comprehensive understanding

**Finding specific rules:**
- By impact: `grep -l "impact: CRITICAL" references/`
- By tag: `grep -l "tags:.*pattern" references/`
- By topic: Search filenames by prefix (`pattern-`, `meta-`, `compose-`, `const-`, `rewrite-`, `org-`, `perf-`, `test-`)
...
For comprehensive details on all 42 rules with full examples, read [references/full-guide.md](references/full-guide.md) when:
- You need the complete context for a specific rule
- The quick reference isn't sufficient
- You're debugging a complex pattern issue
