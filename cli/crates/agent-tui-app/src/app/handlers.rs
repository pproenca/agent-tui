...
use crate::app::recording::RecordingError;
use crate::app::recording::StartRecordingRequest;
use crate::app::recording::start_recording;
use crate::app::recording::stop_recording;
...
fn resolve_recording_session_id<C: DaemonClient>(ctx: &mut HandlerContext<C>) -> Result<String> {
    let session_id = resolve_attach_session_id(ctx)?;
    let sessions_result = call_no_params(ctx.client, "sessions")?;
    let Some(sessions) = sessions_result.get("sessions").and_then(|v| v.as_array()) else {
        return Err(CliError::new(
            ctx.format,
            "Invalid sessions response from daemon",
            Some(sessions_result.to_pretty_json()),
            super::exit_codes::GENERAL_ERROR,
        )
        .into());
    };
    ...
    let Some(session) = sessions
        .iter()
        .find(|candidate| candidate.str_or("id", "") == session_id)
    else {
        return Err(CliError::new(
            ctx.format,
    ...
    if !session.bool_or("running", false) {
        return Err(CliError::new(
            ctx.format,
...
fn recording_error_to_cli_error(format: OutputFormat, error: RecordingError) -> anyhow::Error {
    #[derive(Serialize)]
    struct RecordingErrorJson<'a> {
        success: bool,
        error: &'a str,
        session_id: &'a str,
        message: &'a str,
    ...
    let json = match &error {
        RecordingError::AlreadyRecording { session_id } => {
            serde_json::to_string(&RecordingErrorJson {
                success: false,
        ...
        RecordingError::NotRecording { session_id } => serde_json::to_string(&RecordingErrorJson {
            success: false,
            error: "not_recording",
            session_id,
            message: &message,
        ...
        .ok(),
        RecordingError::Other(_) => None,
    };
    CliError::new(format, message, json, super::exit_codes::GENERAL_ERROR).into()
}

pub(crate) fn handle_session_record_start<C: DaemonClient>(
    ctx: &mut HandlerContext<C>,
    output_file: Option<PathBuf>,
    foreground: bool,
) -> HandlerResult {
    let session_id = resolve_recording_session_id(ctx)?;
    ...
    let result =
        start_recording(request).map_err(|err| recording_error_to_cli_error(ctx.format, err))?;
    match ctx.format {
        OutputFormat::Json => {
            #[derive(Serialize)]
            struct RecordStartOutput {
                success: bool,
                session_id: String,
                pid: u32,
                gif_path: String,
            ...
            let payload = RecordStartOutput {
                success: true,
                session_id: result.entry.session_id.clone(),
                pid: result.entry.pid,
                gif_path: result.entry.gif_path.display().to_string(),
                tape_path: result.entry.tape_path.display().to_string(),
                mode: result.entry.mode.as_str(),
            };
            println!("{}", serde_json::to_string_pretty(&payload)?);
        }
        OutputFormat::Text => {
            println!(
                ...
                Colors::session_id(&result.entry.session_id)
            );
            println!("  PID: {}", result.entry.pid);
            println!("  GIF: {}", result.entry.gif_path.display());
            println!("  Tape: {}", result.entry.tape_path.display());
            println!("  Mode: {}", result.entry.mode.as_str());
        }
    }
    Ok(())
}

pub(crate) fn handle_session_record_stop<C: DaemonClient>(
    ctx: &mut HandlerContext<C>,
) -> HandlerResult {
    let session_id = resolve_attach_session_id(ctx)?;
    let result =
        stop_recording(&session_id).map_err(|err| recording_error_to_cli_error(ctx.format, err))?;

    match ctx.format {
        OutputFormat::Json => {
            #[derive(Serialize)]
            struct RecordStopOutput {
                success: bool,
                session_id: String,
                pid: u32,
                gif_path: String,
